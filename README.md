...
- 컨테이너가 이미 healthy여도 실행기가 없으면 시작한다. 스크립트 경로·내용 SHA-256·대상 주소·
  작업폴더·자격증명 경로가 같은 기존 프로세스는 재사용한다. 같은 워크스페이스·대상 주소의
  관리형 Runner에서 스크립트나 경로가 바뀌면 정상 종료를 요청하고, 진행 중인 작업과 결과 보고가
  끝난 뒤 새 Runner를 시작해 최초 정상 poll을 확인한다. 동시 시작 호출도 직렬화한다.
  Runner 자체도 같은 대상 주소에 대한 잠금을 사용해 수동 실행과 자동 실행의 중복 claim을 막는다.
- 종료 요청은 실행별 Windows 신호로 전달하며 작업 사이에서만 처리한다. 기본 최대 대기 시간은
  30분이고, 직접 Launcher 호출 시 `-DrainTimeoutSeconds`로 바꿀 수 있다. 시간 초과 시 작업을
  강제 종료하거나 새 프로세스를 만들지 않는다. 종료 요청은 유지되므로 작업 종료 후 같은 시작
  명령을 다시 실행한다. 종료 대기 중인 프로세스를 정상 재사용으로 보고하지 않는다.
- 이 종료 기능이 없는 구버전, 수동 사용자 옵션, 다른 워크스페이스·대상 주소, 여러 프로세스,
  PID·최초 poll·종료 신호를 확인할 수 없는 경우는 보존하고 원인을 표시한다. 구버전 최초 전환은
  별도 통제된 종료가 필요하며 파일을 Pull하는 것만으로 이미 실행 중인 프로세스에 기능이 추가되지는 않는다.
  Runner 자신의 작업에서 경로 교체를 요청하면 자기 작업 완료를 기다리는 교착을 막기 위해 차단한다.
...
종료한다. 공식 자동 시작의 관리형 Runner 교체는 위 정상 종료 절차를 사용하며 일괄 강제 종료하지 않는다.
...
.\scripts\verify-runner-drain.ps1
```

숨김 Windows 프로세스까지 검증하려면 `verify-runner-startup.ps1 -ProcessSmoke`를 사용한다.
가짜 토큰과 빈 응답 전용 임시 loopback 서버로 실제 Runner 시작·동일 PID 재사용·경로/내용 변경 교체를 확인하고
테스트 프로세스만 정리한다. 제품 서버의 Job·자격증명은 사용하지 않는다.
`verify-runner-drain.ps1`은 HTTP와 작업 처리만 파일 장벽으로 대체한 실제 프로세스에서
작업·결과 보고 완료까지 대기, 중복 claim 차단, 시간 초과 보존과 재실행을 검증한다.
