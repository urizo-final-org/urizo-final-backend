 ...
 * 따르고, 저장한 뒤부터 목록에 없는 필드는 꺼짐이다. 이 구분이 없으면 설치 직후
 * 모든 필드가 닫힌 것으로 읽혀 자연어 CMS가 통째로 멎는다.
 */
record NaturalCmsGuardrail(
    ...
    NaturalCmsGuardrail {
        Objects.requireNonNull(allowedFields, "allowedFields is required");
        allowedFields = Map.copyOf(allowedFields);
    }

    /** 아직 아무것도 저장하지 않은 상태. 코드 기본값을 그대로 쓴다. */
    static NaturalCmsGuardrail unconfigured() {
        return new NaturalCmsGuardrail(false, true, Map.of());
    }

    /**
     * 이 대상에 열린 명령 종류.
     *
     * <p>삭제만 관리자가 끌 수 있다. 삭제 명령은 필드를 하나도 싣지 않아
     * 필드 선택으로는 막을 수 없으므로 별도 값이 필요하다.
     */
    Set<String> operations(Set<String> opened) {
        if (allowDelete) {
            return opened;
        }
        Set<String> allowed = new LinkedHashSet<>(opened);
     ...
     * 코드가 나중에 새 필드를 열어도 관리자가 켜기 전까지는 닫혀 있다.
     */
    Set<String> fields(String resourceKey, Set<String> opened) {
        if (!configured || !MANAGED.contains(resourceKey)) {
            return opened;
        }
        Set<String> chosen = allowedFields.getOrDefault(resourceKey, Set.of());
