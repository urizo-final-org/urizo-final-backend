-- AI04-021: the server deployment adapter now has a fixed local target for the frontend
-- repository as well, so the deployment request and dev merge check rows may name either
-- published repository. Everything else in the AI04-016 contract is unchanged.
ALTER TABLE app.coding_handler_result
    DROP CONSTRAINT ck_coding_handler_result_ai04_016_payload,
    ADD CONSTRAINT ck_coding_handler_result_ai04_021_payload CHECK (
        ...
        AND (handler_key <> 'coding.deploy_request'
             OR NOT (payload ? 'deploymentRequestId') OR (
                ...
                AND payload ->> 'repository' IN ('backend', 'frontend')
                AND payload ->> 'candidateSha' = candidate_sha
