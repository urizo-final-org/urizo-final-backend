...
class PublicAnswerComposer {
     ...
     *
     * <p>제약 7종: 근거 한정 · 추가 금지 · 부족하면 부족하다고 말하기 · 덩어리 구조 ·
     * 종료 표시 양방향 고정 · 톤 · 마크다운 금지.
     *
     ...
     *
     * <p><b>규칙 4 개편과 규칙 7(2026-09-10, axms-ai02-014).</b> 산문 한 덩어리로 나오던
     * 답변이 읽기 불편해 장소별 덩어리 구조로 바꿨다. 규칙 7이 함께 필요한 이유는 화면이
     * 마크다운을 렌더링하지 않기 때문이다 — {@code ChatWidget}과 {@code TourPortal}의
     * AnswerCard는 {@code whitespace-pre-line} 평문 문단이라 줄바꿈만 살고 {@code *}·
     * {@code **}·링크 문법은 글자 그대로 노출된다. 구조를 지시하면서 마크다운을 막지 않으면
     * 지금보다 더 지저분해진다. 홈페이지·전화번호를 빼는 것도 같은 이유다 — 근거 카드에 이미
     * 있고, 긴 URL이 줄글처럼 보이게 만든다.
     */
    static final String SYSTEM_PROMPT = """
            ...
            4. 소개할 곳이 여럿이면 한 곳씩 덩어리로 나눈다. 각 덩어리는 이름 한 줄,
